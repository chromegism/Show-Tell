...
class Buffer:
  ...
  def VertexAttribPointer(self, index, vals_per_segment, normalised, _stride, offset):
    if not self.dtype:
      raise ValueError("Buffer has no data and therefore no type")
    
    glVertexAttribPointer(index, int(self.dlen / vals_per_segment), self.dtype, normalised, _stride, ctypes.c_void_p(offset))
